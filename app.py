...
def film(id):
    ...
    if result.fetchone()[0] == None:
        available = 'Ei'
        button = 'yes'
        return render_template("film.html", id=id, name=name, genre=genre, release_year=release_year, description=description, available=available, button=button)
    else:
        available = 'Kyllä'
        return render_template("film.html", id=id, name=name, genre=genre, release_year=release_year, description=description, available=available)
...
def member(username):
    ...
    sql = "SELECT name FROM films WHERE member_id=:id "
    result = db.session.execute(sql, {"id":id})
    films = result.fetchall()
...
def submit_member():
    if request.method == 'POST':
        ...
        sql = 'INSERT INTO members (name, username, password, email) ' \
              'VALUES (:name, :username, :password, :email)'
        
        db.session.execute(sql, {'name':name, 'username':username, 'password':password, 'email':email})
        db.session.commit()
        return render_template('success.html')
